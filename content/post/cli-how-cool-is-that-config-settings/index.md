...
👉 One thing that is important to mention is that we have a similar setting `autoOpenLinksInBrowser` which will open any url output from command in browser, not only the login url. This setting for example might be useful for the `m365 app open` command to automatically open the link to the Azure Portal in the browser from the .m365rc.json file for our current app. 
...
Sometimes we need to add quotes to the values. For example if we have some value with a comma in it. We may want to have the comma in the csv file as a comma and not to br used as a separator to next column. In that case we may turn quotes on by setting:
